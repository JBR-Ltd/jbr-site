      ...
      >
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 3rem", height: 76, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <Link href="/" style={{ display: "flex", alignItems: "center", gap: 12, textDecoration: "none" }}>
            ...
            <span style={{ fontFamily: 'var(--font-display)', fontSize: "1.05rem", letterSpacing: "0.15em", color: "#EDE6D6", fontWeight: 600 }}>
          ...
          <nav style={{ display: "flex", alignItems: "center", gap: 36 }} className="hidden-mobile">
            {links.map((l) => (
              <Link key={l.href} href={l.href} style={{ fontFamily: 'var(--font-display)', fontSize: "0.78rem", letterSpacing: "0.18em", textTransform: "uppercase", fontWeight: 600, color: "rgba(237,230,214,0.78)", textDecoration: "none", transition: "color 0.3s" }}
                onMouseEnter={e => (e.currentTarget.style.color = "#E89B4D")}
                onMouseLeave={e => (e.currentTarget.style.color = "rgba(237,230,214,0.78)")}>
                {l.label}
              </Link>
            ))}
            <Link href="/contact" style={{ fontFamily: 'var(--font-display)', fontSize: "0.78rem", letterSpacing: "0.18em", textTransform: "uppercase", fontWeight: 600, padding: "11px 26px", border: "1px solid #C9853E", color: "#EDE6D6", textDecoration: "none", transition: "all 0.3s" }}
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? "Close menu" : "Open menu"} aria-expanded={menuOpen} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", gap: 6, padding: 8 }} className="show-mobile">
            {[0,1,2].map(i => (
            ...
            {links.map((l, i) => (
              <motion.div key={l.href} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.08, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}>
                <Link href={l.href} onClick={() => setMenuOpen(false)}
                  style={{ fontFamily: 'var(--font-display)', fontSize: "2.4rem", fontWeight: 600, color: "#EDE6D6", textDecoration: "none", transition: "color 0.3s" }}
