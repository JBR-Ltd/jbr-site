...
export default function AboutTeaser() {
  ...
  return (
    <section ref={ref} style={{ position: "relative", background: "#0A0807", padding: "9rem 0", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 3rem" }}>
        <FadeIn><p style={{ fontFamily: "Inter, sans-serif", fontSize: "0.65rem", letterSpacing: "0.38em", textTransform: "uppercase", color: "#C9853E", marginBottom: 48 }}>Who We Are</p></FadeIn>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", marginBottom: "6rem" }} className="two-col">
          <TextReveal tag="h2" delay={0.1} style={{ fontFamily: '"Space Grotesk", sans-serif', fontSize: "clamp(1.9rem,3.5vw,3rem)", fontWeight:500, color: "#EDE6D6" }}>
          ...
          <FadeIn direction="left" delay={0.3} style={{ display: "flex", flexDirection: "column", justifyContent: "flex-end", gap: 20 }}>
            ...
            <a href="/about" style={{ fontFamily: "Inter, sans-serif", fontSize: "0.68rem", letterSpacing: "0.35em", textTransform: "uppercase", color: "rgba(237,230,214,0.55)", textDecoration: "none", display: "flex", alignItems: "center", gap: 10, width: "fit-content", transition: "color 0.3s" }}
              onMouseEnter={e => (e.currentTarget.style.color = "#EDE6D6")} onMouseLeave={e => (e.currentTarget.style.color = "rgba(237,230,214,0.55)")}>
              Full Story <span style={{ display: "inline-block", width: 20, height: 1, background: "currentColor" }} />
            </a>
          </FadeIn>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "2rem", borderTop: "1px solid rgba(201,133,62,0.2)", paddingTop: 48 }}>
          {stats.map((s, i) => (
            <FadeIn key={s.label} delay={0.1 + i * 0.12} direction="up">
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <span style={{ fontFamily: '"Space Grotesk", sans-serif', fontSize: "clamp(2.5rem,5vw,3.8rem)", fontWeight:500, color: "#EDE6D6" }}>{s.value}</span>
                <span style={{ fontFamily: "Inter, sans-serif", fontSize: "0.65rem", letterSpacing: "0.3em", textTransform: "uppercase", color: "#C9853E" }}>{s.label}</span>
                <span style={{ fontFamily: "Inter, sans-serif", fontSize: "0.75rem", color: "rgba(237,230,214,0.3)" }}>{s.sub}</span>
              </div>
            </FadeIn>
          ))}
        </div>
