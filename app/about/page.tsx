...
export default function AboutPage() {
  ...
  return (
    <div style={{ background: "#0A0807" }}>
      ...
      <section style={{ padding:"9rem 0", background:"#0A0807" }} className="section-pad">
        <div style={{ maxWidth:1280, margin:"0 auto", padding:"0 3rem" }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1.4fr", gap:"6rem", alignItems:"start" }} className="story-grid">
            <FadeIn>
              <p style={{ fontFamily:"var(--font-sans)", fontSize:"0.65rem", letterSpacing:"0.38em", textTransform:"uppercase", color:"#C9853E", marginBottom:32 }}>Our Story</p>
              <div className="hr-accent" />
            </FadeIn>
            <div style={{ display:"flex", flexDirection:"column", gap:28 }}>
              <TextReveal tag="h2" style={{ fontFamily:'var(--font-display)', fontSize:"clamp(1.6rem,3vw,2.4rem)", fontWeight:500, color:"#EDE6D6" }}>
              ...
              {["JBR Limited was born out of a recurring frustration: the gap between the technology African markets deserve and what they typically receive. Too many solutions are built elsewhere and adapted here, poorly.",
                ...
                <FadeIn key={i} delay={0.1 + i * 0.1}>
                  <p style={{ fontFamily:"var(--font-sans)", fontSize:"0.88rem", color:"rgba(237,230,214,0.82)", lineHeight:1.95 }}>{p}</p>
                </FadeIn>
              ))}
            </div>
          </div>
      ...
      {/* ── Values ── */}
      <section style={{ padding:"7rem 0", background:"#2A1C10" }} className="section-pad">
        <div style={{ maxWidth:1280, margin:"0 auto", padding:"0 3rem" }}>
          <FadeIn style={{ marginBottom:64 }}>
            <p style={{ fontFamily:"var(--font-sans)", fontSize:"0.65rem", letterSpacing:"0.38em", textTransform:"uppercase", color:"#C9853E", marginBottom:20 }}>Our Values</p>
            <TextReveal tag="h2" style={{ fontFamily:'var(--font-display)', fontSize:"clamp(1.7rem,3vw,2.5rem)", fontWeight:500, color:"#EDE6D6" }}>
            ...
            {values.map((v, i) => (
              <FadeIn key={v.title} delay={i * 0.09}>
                <div style={{ padding:"2.8rem", background:"#2A1C10", borderBottom:"1px solid rgba(201,133,62,0.15)", transition:"background 0.4s" }}
                  ...
                  <span style={{ fontFamily:'var(--font-display)', fontSize:"2.2rem", fontWeight:500, color:"rgba(201,133,62,0.5)", display:"block", marginBottom:16 }}>0{i+1}</span>
                  <h3 style={{ fontFamily:'var(--font-display)', fontSize:"1.6rem", fontWeight:400, color:"#EDE6D6", marginBottom:12 }}>{v.title}</h3>
                  <p style={{ fontFamily:"var(--font-sans)", fontSize:"0.8rem", color:"rgba(237,230,214,0.8)", lineHeight:1.85 }}>{v.desc}</p>
                </div>
              </FadeIn>
            ))}
          </div>
      ...
      <section style={{ padding:"9rem 0", background:"#0A0807" }} className="section-pad">
        <div style={{ maxWidth:1280, margin:"0 auto", padding:"0 3rem" }}>
          <FadeIn style={{ marginBottom:72 }}>
            <p style={{ fontFamily:"var(--font-sans)", fontSize:"0.65rem", letterSpacing:"0.38em", textTransform:"uppercase", color:"#C9853E", marginBottom:20 }}>The Team</p>
            <TextReveal tag="h2" style={{ fontFamily:'var(--font-display)', fontSize:"clamp(1.7rem,3vw,2.5rem)", fontWeight:500, color:"#EDE6D6" }}>
          ...
          {/* Mobile carousel, swipeable, avoids the "one long column" feel */}
