    ...
    <div style={{ background:"#0A0807" }}>
      ...
      <section ref={heroRef} style={{ position:"relative", minHeight:"100vh", display:"flex", alignItems:"flex-end", overflow:"hidden" }}>
        <motion.div style={{ y:heroY, position:"absolute", inset:0 }}>
          ...
          {/* Animated grid of property squares, pure CSS animation instead
              of 48 looping Framer Motion instances, which was a real
              main-thread cost running forever in the background */}
      ...
      <section style={{ padding:"9rem 0", background:"#0A0807" }} className="section-pad">
        <div style={{ maxWidth:1280, margin:"0 auto", padding:"0 3rem" }}>
          <FadeIn style={{ marginBottom:64 }}>
            <p style={{ fontFamily:"var(--font-sans)", fontSize:"0.65rem", letterSpacing:"0.38em", textTransform:"uppercase", color:"#C9853E", marginBottom:16 }}>Venture 01</p>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"5rem", alignItems:"start", marginBottom:"6rem" }} className="venture-intro">
            <div>
              ...
              <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginBottom:28 }}>
                {["Real Estate","PropTech","Nigeria","2025"].map(t => (
                  <span key={t} style={{ fontFamily:"var(--font-sans)", fontSize:"0.62rem", letterSpacing:"0.25em", textTransform:"uppercase", padding:"5px 12px", border:"1px solid rgba(201,133,62,0.5)", color:"rgba(237,230,214,0.82)" }}>{t}</span>
                ))}
              </div>
            </div>
            <FadeIn delay={0.2} style={{ display:"flex", flexDirection:"column", gap:20 }}>
              ...
              <div style={{ display:"flex", gap:16, marginTop:8, flexWrap:"wrap" }}>
                <a href="#" style={{ fontFamily:"var(--font-sans)", fontSize:"0.68rem", letterSpacing:"0.35em", textTransform:"uppercase", padding:"12px 28px", background:"#C9853E", color:"#EDE6D6", textDecoration:"none", transition:"all 0.35s" }}
                ...
                <span style={{ fontFamily:"var(--font-sans)", fontSize:"0.65rem", letterSpacing:"0.2em", textTransform:"uppercase", padding:"12px 16px", color:"rgba(201,133,62,0.8)", display:"flex", alignItems:"center" }}>
                  Live · 2025
                </span>
              </div>
            </FadeIn>
                ...
                {features.map((f, i) => (
                  <FadeIn key={f.title} delay={i * 0.07}>
                    <div style={{ padding:"2.4rem", background:"#0A0807", transition:"background 0.4s" }}
                      ...
                      <h3 style={{ fontFamily:'var(--font-display)', fontSize:"1.4rem", fontWeight:400, color:"#EDE6D6", marginBottom:10 }}>{f.title}</h3>
                      <p style={{ fontFamily:"var(--font-sans)", fontSize:"0.78rem", color:"rgba(237,230,214,0.65)", lineHeight:1.85 }}>{f.desc}</p>
                    </div>
                  </FadeIn>
                ))}
              </div>
      ...
      <section style={{ padding:"7rem 0 9rem", background:"#2A1C10" }} className="section-pad">
        <div style={{ maxWidth:1280, margin:"0 auto", padding:"0 3rem", textAlign:"center", display:"flex", flexDirection:"column", alignItems:"center", gap:28 }}>
          <FadeIn><p style={{ fontFamily:"var(--font-sans)", fontSize:"0.65rem", letterSpacing:"0.38em", textTransform:"uppercase", color:"#C9853E" }}>Coming Next</p></FadeIn>
          <TextReveal tag="h2" style={{ fontFamily:'var(--font-display)', fontSize:"clamp(1.9rem,3.5vw,2.9rem)", fontWeight:500, color:"#EDE6D6", maxWidth:600 }}>
            More ventures. Already in motion.
          </TextReveal>
          <FadeIn delay={0.2}><p style={{ fontFamily:"var(--font-sans)", fontSize:"0.82rem", color:"rgba(237,230,214,0.65)", maxWidth:400, lineHeight:1.85 }}>We're building more. We'll announce when the work is ready, not before.</p></FadeIn>
        </div>
      </section>
    </div>
  );
