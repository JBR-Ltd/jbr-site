...
export default function ServicesPage() {
  ...
  return (
    <div style={{ background:"#0A0807" }}>
      ...
      {/* Intro */}
      <section style={{ padding:"7rem 0", background:"#0A0807" }}>
        <div style={{ maxWidth:1280, margin:"0 auto", padding:"0 3rem" }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1.5fr", gap:"6rem" }} className="intro-grid">
            ...
            <FadeIn delay={0.2}>
              <p style={{ fontFamily:"Inter,sans-serif", fontSize:"1rem", color:"rgba(237,230,214,0.55)", lineHeight:1.95, maxWidth:520 }}>
      ...
      <section style={{ padding:"0 0 9rem", background:"#0A0807" }}>
              ...
              <div style={{ borderTop:"1px solid rgba(201,133,62,0.25)", cursor:"none" }} onClick={() => setOpen(open===i ? null : i)}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"2rem 0", gap:24 }}>
                  <div style={{ display:"flex", alignItems:"baseline", gap:28, flex:1 }}>
                    <span style={{ fontFamily:'"Space Grotesk",sans-serif', fontSize:"0.9rem", color:"rgba(201,133,62,0.55)", letterSpacing:"0.2em", minWidth:28 }}>{s.num}</span>
                    <h2 style={{ fontFamily:'"Space Grotesk",sans-serif', fontSize:"clamp(1.4rem,2.8vw,2.2rem)", fontWeight:500, color:"#EDE6D6", transition:"color 0.3s" }}
                ...
                <motion.div initial={false} animate={{ height: open===i ? "auto" : 0, opacity: open===i ? 1 : 0 }}
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"3rem", paddingBottom:"2.5rem", paddingLeft:56 }} className="acc-grid">
                    <p style={{ fontFamily:"Inter,sans-serif", fontSize:"0.85rem", color:"rgba(237,230,214,0.5)", lineHeight:1.9 }}>{s.desc}</p>
                    <ul style={{ listStyle:"none", display:"flex", flexDirection:"column", gap:10 }}>
                      {s.details.map(d => (
                        <li key={d} style={{ fontFamily:"Inter,sans-serif", fontSize:"0.78rem", color:"rgba(237,230,214,0.4)", display:"flex", alignItems:"center", gap:12 }}>
        ...
        <style>{`.acc-grid{@media(max-width:768px){grid-template-columns:1fr !important;}}`}</style>
      </section>
      ...
      <section style={{ padding:"0 0 9rem", background:"#0A0807" }}>
        <div style={{ maxWidth:1280, margin:"0 auto", padding:"0 3rem", textAlign:"center", display:"flex", flexDirection:"column", alignItems:"center", gap:32 }}>
          <TextReveal tag="h2" style={{ fontFamily:'"Space Grotesk",sans-serif', fontSize:"clamp(1.9rem,3.5vw,2.9rem)", fontWeight:500, color:"#EDE6D6" }}>
