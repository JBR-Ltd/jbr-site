...
export default function ContactPage() {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    padding:"14px 0", fontFamily:"Inter,sans-serif", fontSize:"0.88rem", color:"#EDE6D6",
  ...
  return (
    <div style={{ background:"#0A0807" }}>
      ...
      <section style={{ padding:"7rem 0 9rem", background:"#0A0807" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:48 }}>
              ...
              {[
                ...
                <FadeIn key={item.label} delay={0.1}>
                  <div>
                    <p style={{ fontFamily:"Inter,sans-serif", fontSize:"0.62rem", letterSpacing:"0.35em", textTransform:"uppercase", color:"#C9853E", marginBottom:8 }}>{item.label}</p>
                    {item.href
                      ? <a href={item.href} style={{ fontFamily:'"Space Grotesk",sans-serif', fontSize:"1.4rem", fontWeight:500, color:"#EDE6D6", textDecoration:"none", transition:"color 0.3s" }}
                          onMouseEnter={e => (e.currentTarget.style.color="#C9853E")} onMouseLeave={e => (e.currentTarget.style.color="#EDE6D6")}>{item.value}</a>
                      : <p style={{ fontFamily:'"Space Grotesk",sans-serif', fontSize:"1.4rem", fontWeight:500, color:"#EDE6D6" }}>{item.value}</p>
                    }
                  </div>
                </FadeIn>
              ))}
              ...
              <FadeIn delay={0.2}>
                <div>
                  <p style={{ fontFamily:"Inter,sans-serif", fontSize:"0.62rem", letterSpacing:"0.35em", textTransform:"uppercase", color:"#C9853E", marginBottom:16 }}>Socials</p>
                  <div style={{ display:"flex", gap:20 }}>
                    {[{handle:"LinkedIn", link:"https://www.linkedin.com/company/rello-jbr/"},{handle:"X", link:"https://x.com/rello_online"},{handle:"Instagram",link:"https://www.instagram.com/rello.jbr?utm_source=qr"}, {handle:"Facebook",link:"https://www.facebook.com/profile.php?id=61590472272578"}].map(s => (
                      <a key={s.handle} href={s.link} style={{ fontFamily:"Inter,sans-serif", fontSize:"0.75rem", color:"rgba(237,230,214,0.4)", textDecoration:"none", transition:"color 0.3s" }}
                        onMouseEnter={e => (e.currentTarget.style.color="#EDE6D6")} onMouseLeave={e => (e.currentTarget.style.color="rgba(237,230,214,0.4)")}>
                        {s.handle}
                      </a>
                    ))}
                  </div>
            ...
            <FadeIn delay={0.15} direction="left">
              {submitted ? (
                <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.8, ease:[0.16,1,0.3,1] }}
                  ...
                  <span style={{ fontFamily:'"Space Grotesk",sans-serif', fontSize:"4rem", fontWeight:500, color:"#C9853E" }}>✓</span>
                  <h3 style={{ fontFamily:'"Space Grotesk",sans-serif', fontSize:"2rem", fontWeight:500, color:"#EDE6D6" }}>Message received.</h3>
                  <p style={{ fontFamily:"Inter,sans-serif", fontSize:"0.82rem", color:"rgba(237,230,214,0.45)", lineHeight:1.85 }}>We'll be in touch within two business days.</p>
                </motion.div>
              ) : (
                <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:36 }}>
                  {/* Inquiry type */}
                  <div>
                    <p style={{ fontFamily:"Inter,sans-serif", fontSize:"0.62rem", letterSpacing:"0.35em", textTransform:"uppercase", color:"rgba(237,230,214,0.35)", marginBottom:16 }}>Nature of Inquiry</p>
                    <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                      {inquiries.map(q => (
                        <button key={q} type="button" onClick={() => setSelected(q)}
                          style={{ fontFamily:"Inter,sans-serif", fontSize:"0.65rem", letterSpacing:"0.2em", textTransform:"uppercase", padding:"7px 14px", border:`1px solid ${selected===q?"#C9853E":"rgba(201,133,62,0.35)"}`, background:selected===q?"#C9853E":"transparent", color:selected===q?"#EDE6D6":"rgba(237,230,214,0.45)", cursor:"none", transition:"all 0.3s" }}>
                          {q}
                        </button>
                      ))}
                    </div>
                  ...
                  {[
                    ...
                    <div key={f.label}>
                      <label style={{ fontFamily:"Inter,sans-serif", fontSize:"0.62rem", letterSpacing:"0.35em", textTransform:"uppercase", color:"rgba(237,230,214,0.35)", display:"block", marginBottom:8 }}>{f.label}</label>
                  ...
                  <div>
                    <label style={{ fontFamily:"Inter,sans-serif", fontSize:"0.62rem", letterSpacing:"0.35em", textTransform:"uppercase", color:"rgba(237,230,214,0.35)", display:"block", marginBottom:8 }}>Your Message</label>
                  ...
                  <button type="submit" style={{ fontFamily:"Inter,sans-serif", fontSize:"0.68rem", letterSpacing:"0.35em", textTransform:"uppercase", padding:"16px 36px", background:"#C9853E", color:"#EDE6D6", border:"none", cursor:"none", transition:"all 0.35s", alignSelf:"flex-start" }}
